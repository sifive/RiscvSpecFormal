...
a jump to a register, a jump to immediate value, a system call, a
...
(such as decode, execute, etc) with the specification they must be
proven against. For such a microarchitecture, it is important that
each functional unit handles several instructions, where the inputs
for the functional units are multiplexed from different sources and
transformed based on the instruction. This was the main reason for
such an organization. This organization of separating the semantics of
an instruction into a *ExecContextPkt* transformer to feed a
functional unit, the generic functionality of the functional unit and
a transformation of the output of the functional unit to an
*UpdateContextPkt* does not overly impose any burden on the
readability or the understandability of the RISC-V ISA specification, but
eases the formal verification cost significantly.
