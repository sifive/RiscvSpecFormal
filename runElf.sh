...
[[ $haskellSim == 1 ]] && execute "time ./HaskellGen/SimMain boot_rom=boot_ROM_RV${xlen}.hex $interactive $noprint testfile=$hexfile tohost_address:$tohost_address xlen@${xlen} $signature $sign_size $debug $interrupts $profile $heapdump > $dump/$base.out"
[[ $coqSim     == 1 ]] && execute "time ./HaskellGen/CoqMain boot_rom=boot_ROM_RV${xlen}.hex $interactive --debug $noprint testfile=$hexfile tohost_address:$tohost_address xlen@${xlen} $signature $sign_size $debug $interrupts > $dump/$base.out"
